...
export async function deployProject(
  ...
  try {
    ...
    const result = await deploymentService.deployAndWait(
      slug,
      project.files,
      (progress) => {
        projectsStore.update(project.id, { deploymentStatus: progress.status });
        eventBus.emitProjectEvent(
          project.id,
          `deployment.${progress.status.toLowerCase()}`,
          {
    ...
    if (result.status === "READY") {
      ...
      projectsStore.update(project.id, {
        ...
        deployError: "Deployment failed or timed out",
      });
